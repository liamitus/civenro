...
async function main() {
  ...
  const bill =
    candidates.find((b) => /Safeguard American Voter|SAVE America/i.test(b.title)) ||
    candidates[0];
  console.log(`Bill: ${bill.title}`);
  console.log(`Versions with text: ${bill.textVersions.map((v) => v.versionCode).join(", ")}`);
  ...
  console.log(`Using version: ${preferred.versionCode} (${preferred.fullText!.length} chars)`);
